...
So, in order to optimize a system, one must focus on common sense (a \textit{sadly} valuable quality).
  ...
  \item \textbf{Compilation} techniques
        \begin{itemize}
          ...
          \item \textbf{Instructions scheduling}
                \begin{itemize}
                  \item the compiler tries to avoid that correlating instruction are too close
...
The pipeline is now constituted by \(6\) stages, normally represented in Figure~\ref{fig:mips-pipeline-stages-issue}.
...
\begin{itemize}
  \item The \textbf{accuracy}, measured in terms of percentage of incorrect predictions given
...
\begin{enumerate}
  ...
  \item \textbf{Data flow}: the actual flow of data values among instructions that produce the correct results and consumes them
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Execution} - operands are operated upon
        \begin{itemize}
          ...
          \item when both operands are in reservation station, the instruction is executed
...
\begin{itemize}
  ...
  \item \textbf{User requested} and \textbf{Coerced}:
        \begin{itemize}
          \item User requested are predictable, they are treated as exceptions because they use the same mechanisms that are used to save and restore the state and are handled after the instruction has completed
...
This problem can be solved by tagging instructions in pipeline as \inlinequote{cause exceptions or not}, and waiting until the end of \texttt{ME} stage to flag exceptions.
Then:
...
The cache, however, needs to introduce a new problem: \textbf{cache coherency}.
