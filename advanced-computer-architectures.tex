...
\begin{enumerate}
  \item Computer system user
        \begin{itemize}
          \item \textbf{le} for program execution
...
Almost all processors with delayed branch have a single delay slot, as it's difficult for the compiler to fill more than one slot.
...
The operand buffers are called \textbf{Reservation Stations} (or \textit{RS}):
each instruction is also an entry to a \textit{RS} and its operands are replaced by values or pointers (a technique known as \textbf{Implicit Register Renaming}) in order to avoid \textit{WAR} and \textit{RAW} hazards.
...
\begin{itemize}
  \item \textbf{Register File} and the \textbf{Store} buffers have a \textit{Value} (\texttt{V}) and a \textit{Pointer} (\texttt{Q}) field
        ...
        \end{itemize}
  \item \textbf{Load} and \textbf{Store} buffers have an \textit{Address} (\texttt{A}) field, with the former having also a \textit{Busy} field (\texttt{BUSY})
        \begin{itemize}
...
\texttt{LOAD} and \texttt{STORE} instructions go through a functional unit for effective computation before proceeding to their respective load and store buffers.
...
The \textbf{compiler} has to schedule the instructions (via \textbf{static scheduling}) to maximize the parallel execution:
...
A definition of \textbf{parallel architectures}, as defined by \textit{Almasi} and \textit{Gottlieb} in 1989 is:
...
This solution requires broadcast, since the cache controllers must know the sharing status of all the data blocks and it's particularly suitable for \textbf{Centralized Shared Memory Architectures}, especially for small scale multiprocessors with single \texttt{BUS}.
...
\bigskip
Since every \texttt{BUS} transaction check the cache address tags, this operation can interfere with the processor operations, causing stall when the variable is not available in the cache.
...
\begin{table}[htbp]
  ...
  \scalebox{0.75}{
    \begin{tabular}{r|c|c|c|c}
      ...
      \textit{A write on this line}          & access the \texttt{BUS} & access the \texttt{BUS} & access the \texttt{BUS},  update the cache & direct access to the \texttt{BUS}
    \end{tabular}
  }
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{\textwidth}
    ...
    \begin{tikzpicture}[auto, node distance=4cm, >=Triangle]
      ...
      \node [state, initial, initial right, initial text={\texttt{READ} miss, not shared}, minimum size=1cm, right=of M](E) {\texttt{E}};
      \node [state, initial, initial left, initial text={\texttt{READ} miss, shared}, minimum size=1cm, below=of M ](S) {\texttt{S}};
...
\begin{itemize}
  ...
  \item \textbf{Trend towards fuller vector support in microprocessors}
        \begin{itemize}
          ...
          \item support of double precision \textit{(64 bits floating point)}
        \end{itemize}
\end{itemize}
