...
\begin{enumerate}
  \item Computer system user
        \begin{itemize}
          \item \textbf{minimize elapsed time} for program execution
          \item \(\texttt{execution time} = \texttt{time end } - \texttt{ time start}\)
        \end{itemize}
  \item Computer center manager
        \begin{itemize}
          \item \textbf{maximize completion rate}
...
Temporary storage \textit{(in the form of shadow registers and store buffers)} is then needed to hold all results before commit
...
% I am really sorry for this abomination
\begin{table}[htbp]
  \centering
  \scalebox{0.8}{
    \Rotatebox{90}{
